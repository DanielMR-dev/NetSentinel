...
use pnet::packet::icmpv6::{
    ndp::NeighborAdvertPacket, ndp::RouterAdvertPacket, Icmpv6Packet, Icmpv6Types,
    MutableIcmpv6Packet,
};
...
pub async fn discover_ipv6_hosts(
    ...
    let src_ipv6 = iface
        ...
        .ok_or_else(|| {
            ScanError::NoInterfaceForIp(format!("Interface {} has no IPv6 address", iface.name))
        })?;

    let _ = event_tx.send(AppEvent::ScanLog {
        level: "info".to_string(),
        message: format!(
            "Starting IPv6 Multicast discovery on {} ({})",
            iface.name, src_ipv6
        ),
        target: None,
    ...
    let devices = tokio::task::spawn_blocking(move || -> Result<Vec<Device>, ScanError> {
        ...
        let (mut sender, mut receiver) = match datalink::channel(&iface_clone, Default::default()) {
            ...
            Err(e) => {
                return Err(ScanError::PermissionDenied(format!(
        ...
        let mut ipv6_packet =
            MutableIpv6Packet::new(ethernet_packet.payload_mut()).ok_or_else(|| {
                ScanError::PacketError("Buffer too small for IPv6 header".to_string())
            })?;
        ...
        let mut icmp_packet =
            MutableIcmpv6Packet::new(ipv6_packet.payload_mut()).ok_or_else(|| {
                ScanError::PacketError("Buffer too small for ICMPv6 header".to_string())
            })?;
        ...
        // Send multicast ping
        if let Some(send_res) = sender.send_to(ethernet_packet.packet(), None) {
            if let Err(e) = send_res {
                return Err(ScanError::DatalinkError(format!(
                    "Failed to send ICMPv6 multicast ping: {}",
                    e
                )));
            }
        }
                                ...
                                if let Some(icmpv6) = Icmpv6Packet::new(ipv6.payload()) {
                                    match icmpv6.get_icmpv6_type() {
                                        ...
                                        Icmpv6Types::NeighborAdvert => {
                                            if let Some(_) =
                                                NeighborAdvertPacket::new(icmpv6.packet())
                                            {
                                                is_new_device = true;
                                            }
                                        }
                                        Icmpv6Types::RouterAdvert => {
                                            if let Some(_) =
                                                RouterAdvertPacket::new(icmpv6.packet())
                                            {
                                                is_new_device = true;
                                            }
                                        }
    ...
    .await
    .map_err(|e| ScanError::NetworkError(format!("Thread crashed: {}", e)))??;
