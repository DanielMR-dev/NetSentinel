...
pub async fn discover_mdns() -> Result<Vec<Device>, ScanError> {
    ...
    let socket = UdpSocket::bind(&bind_addr).await.map_err(|e| {
        ScanError::NetworkError(format!("Failed to bind mDNS socket: {}", e))
    })?;
    ...
    if let Err(e) = socket.send_to(&query, &mdns_addr).await {
        ...
        return Err(ScanError::NetworkError(format!("Failed to send mDNS query: {}", e)));
    }
...
pub async fn discover_netbios(broadcast_ip: Ipv4Addr) -> Result<Vec<Device>, ScanError> {
    ...
    let socket = UdpSocket::bind(&bind_addr).await.map_err(|e| {
        ScanError::NetworkError(format!("Failed to bind NBNS socket: {}", e))
    })?;
    ...
    if let Err(e) = socket.send_to(&query, &nbns_addr).await {
        ...
        return Err(ScanError::NetworkError(format!("Failed to send NBNS query: {}", e)));
    }
...
fn parse_nbns_response(buf: &[u8]) -> (Option<String>, Option<String>) {
    ...
    for _ in 0..num_names {
        ...
        if record_type == 0x00 && !is_group && hostname.is_none() {
            ...
            let name_end = name_bytes.iter().position(|&b| b == 0x20 || b == 0x00).unwrap_or(15);
    ...
    if offset + 6 <= buf.len() {
        ...
        mac = Some(format!("{:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}", m[0], m[1], m[2], m[3], m[4], m[5]));
    }
