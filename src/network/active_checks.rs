...
pub async fn run_active_checks(ip: &str, ports: &[u16]) -> Vec<ActiveCheckResult> {
    ...
    if ports.contains(&21) || ports.contains(&22) || ports.contains(&23) {
        ...
        debug!("Target {} has auth ports open, queuing default creds check.", ip);
    }
...
async fn check_eternalblue(ip: &str) -> Result<ActiveCheckResult, ScanError> {
    ...
    let mut stream = match tokio::time::timeout(Duration::from_secs(3), TcpStream::connect(&addr)).await {
        Ok(Ok(s)) => s,
        _ => return Err(ScanError::NetworkError("Failed to connect to port 445".to_string())),
    };
    ...
    let smb_negotiate_payload = [
        ...
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Client/Process/Tree/User IDs
        0x00, 0x00, 0x00, 0x00, 0x00, 0x0f, 0x00, // Word count, byte count
        0x02, 0x4e, 0x54, 0x20, 0x4c, 0x4d, 0x20, 0x30, 0x2e, 0x31, 0x32, 0x00, // Dialect: "NT LM 0.12"
    ];

    if let Err(_) = stream.write_all(&smb_negotiate_payload).await {
        return Err(ScanError::NetworkError("Failed to write SMB payload".to_string()));
    }
