...
Use only the tokens necessary for the user request: no filler, no repeated context, no excessive summaries. Give subagents concise, bounded prompts that explicitly tell them to load Backend and Frontend skills, avoid hallucination, do only the assigned task, and return minimal actionable output.
